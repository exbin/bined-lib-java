...
import org.exbin.framework.deltahex.operation.command.HexCommandType;
import org.exbin.framework.deltahex.operation.command.EditCharDataCommand;
        ...
        if (clipboard.isDataFlavorAvailable(appDataFlavor)) {
            try {
                ...
                if (object instanceof HexadecimalData) {
                    ...
                    HexadecimalData pastedData = (HexadecimalData) object;
                    long dataSize = pastedData.getDataSize();
                    long insertionPosition = dataPosition;
                    if (hexadecimal.getEditationMode() == Hexadecimal.EditationMode.OVERWRITE) {
                        HexadecimalData modifiedData = pastedData;
            ...
            try {
                ...
                if (insertedData instanceof String) {
                    ...
                    HexadecimalData pastedData = new MemoryHexadecimalData(bytes);
                    long insertionPosition = dataPosition;
                    if (hexadecimal.getEditationMode() == Hexadecimal.EditationMode.OVERWRITE) {
                        HexadecimalData modifiedData = pastedData;
                        long toReplace = dataSize;
