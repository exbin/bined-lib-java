...
public class DefaultCodeAreaPainter implements CodeAreaPainter {
    ...
    protected final CodeAreaFX codeArea;
    ...
    public Node paintOverall() {
        ...
        if ((decorationMode & CodeAreaFX.DECORATION_LINENUM_LINE) > 0) {
            int lineX = codeRect.x - 1 - codeArea.getLineNumberSpace() / 2;
            Line lineNumLine = new Line(lineX, compRect.y, lineX, codeRect.y);
            lineNumLine.setFill(codeArea.getDecorationLineColor());
            group.getChildren().add(lineNumLine);
        }
        if ((decorationMode & CodeAreaFX.DECORATION_HEADER_LINE) > 0) {
            Line headerLine = new Line(compRect.x, codeRect.y - 1, compRect.x + compRect.width, codeRect.y - 1);
            headerLine.setFill(codeArea.getDecorationLineColor());
            group.getChildren().add(headerLine);
        }
        if ((decorationMode & CodeAreaFX.DECORATION_BOX) > 0) {
            Line decorationBoxLine = new Line(codeRect.x - 1, codeRect.y - 1, codeRect.x + codeRect.width, codeRect.y - 1);
            decorationBoxLine.setFill(codeArea.getDecorationLineColor());
            group.getChildren().add(decorationBoxLine);
        }
...
//
//            if (codeArea.getBackgroundMode() == CodeAreaFX.BackgroundMode.GRIDDED) {
...
//            CodeAreaFX.CharRenderingMode charRenderingMode = codeArea.getCharRenderingMode();
//
...
//                if (charRenderingMode != CodeAreaFX.CharRenderingMode.LINE_AT_ONCE) {
//                    char currentChar = ' ';
...
//                        continue;
//                    }
//                    if (charRenderingMode == CodeAreaFX.CharRenderingMode.AUTO && monospaceFont) {
...
//                        if (charRenderingMode == CodeAreaFX.CharRenderingMode.TOP_LEFT) {
...
//        }
//        if ((decorationMode & CodeAreaFX.DECORATION_PREVIEW_LINE) > 0) {
...
//        }
    ...
    public Color getHeaderPositionColor(int byteOnLine, int charOnLine) {
        return codeArea.getTextColor();
    }

    @Override
    public Node paintBackground() {
...
//            }
//        }
//        if (codeArea.getBackgroundMode() != CodeAreaFX.BackgroundMode.NONE) {
//// TODO            g.setColor(mainColors.getBackgroundColor());
...
//        if (codeArea.getBackgroundMode() == CodeAreaFX.BackgroundMode.STRIPPED || codeArea.getBackgroundMode() == CodeAreaFX.BackgroundMode.GRIDDED) {
//// TODO            g.setColor(stripColors.getBackgroundColor());
//
...
// TODO        g.setColor(codeArea.getForeground());
...
//            if (codeArea.getCharRenderingMode() == CodeAreaFX.CharRenderingMode.LINE_AT_ONCE) {
...
//        int decorationMode = codeArea.getDecorationMode();
//        if ((decorationMode & CodeAreaFX.DECORATION_LINENUM_LINE) > 0) {
//            // TODO g.setColor(codeArea.getDecorationLineColor());
//            int lineX = codeRect.x - 1 - codeArea.getLineNumberSpace() / 2;
...
//        }
    ...
    @Override
    public Node paintMainArea() {
        PaintData paintData = new PaintData(codeArea);
        return paintMainArea(paintData);
    }

    public Node paintMainArea(PaintData paintData) {
        if (paintData.viewMode != ViewMode.TEXT_PREVIEW && codeArea.getBackgroundMode() == CodeAreaFX.BackgroundMode.GRIDDED) {
// TODO            g.setColor(paintData.alternateColors.getBackgroundColor());
        ...
        do {
            ...
            if (paintData.viewMode != ViewMode.CODE_MATRIX) {
                for (int byteOnLine = paintData.visiblePreviewStart; byteOnLine < Math.min(paintData.visiblePreviewEnd, lineBytesLimit); byteOnLine++) {
                    ...
                    if (paintData.showUnprintableCharacters || paintData.charRenderingMode == CodeAreaFX.CharRenderingMode.LINE_AT_ONCE) {
...
//                g.drawLine(lineX, paintData.codeSectionRect.y, lineX, paintData.codeSectionRect.y + paintData.codeSectionRect.height);
    ...
    public void paintLineText(Graphics g, int linePositionX, int linePositionY, PaintData paintData) {
        ...
        for (int charOnLine = paintData.visibleCharStart; charOnLine < paintData.visibleCharEnd; charOnLine++) {
            ...
            int currentCharWidth = 0;
            ColorsGroup.ColorType colorType = ColorsGroup.ColorType.TEXT;
            if (paintData.charRenderingMode != CodeAreaFX.CharRenderingMode.LINE_AT_ONCE) {
                char currentChar = ' ';
                if (paintData.showUnprintableCharacters) {
                ...
                if (paintData.charRenderingMode == CodeAreaFX.CharRenderingMode.AUTO && paintData.monospaceFont) {
                    // Detect if character is in unicode range covered by monospace fonts
                    if (currentChar > MIN_MONOSPACE_CODE_POINT && (int) currentChar < MAX_MONOSPACE_CODE_POINT
                            && currentChar != INV_SPACE_CODE_POINT
                            && currentChar != EXCEPTION1_CODE_POINT && currentChar != EXCEPTION2_CODE_POINT) {
            ...
            if (sequenceBreak) {
                ...
                if (!nativeWidth) {
                    renderOffset = charOnLine + 1;
                    if (paintData.charRenderingMode == CodeAreaFX.CharRenderingMode.TOP_LEFT) {
                        g.drawChars(
...
//            return null;
//        }
//        if (((paintData.backgroundMode == CodeAreaFX.BackgroundMode.STRIPPED || paintData.backgroundMode == CodeAreaFX.BackgroundMode.GRIDDED) && (paintData.line & 1) > 0)
//                || (paintData.backgroundMode == CodeAreaFX.BackgroundMode.GRIDDED && ((byteOnLine & 1) > 0)) && section == Section.CODE_MATRIX) {
...
//            // TODO g.setColor(codeArea.getCursorColor());
//            if (renderingMode == CodeAreaCaret.CursorRenderingMode.XOR) {
//                // TODO g.setXORMode(Color.WHITE);
//            }
//
...
//                CodeAreaFX.ScrollPosition scrollPosition = codeArea.getScrollPosition();
//                g.fillRect(x, y, width, height);
//                // TODO g.setColor(codeArea.getNegativeCursorColor());
//                Rectangle codeRect = codeArea.getCodeSectionRectangle();
...
//    }
    ...
    protected static class PaintData {
        ...
        protected CodeAreaFX.BackgroundMode backgroundMode;
        protected Rectangle codeSectionRect;
        protected CodeAreaFX.ScrollPosition scrollPosition;
        ...
        protected CodeAreaFX.CharRenderingMode charRenderingMode;
        ...
        public PaintData(CodeAreaFX codeArea) {
        ...
        public CodeAreaFX.BackgroundMode getBackgroundMode() {
        ...
        public CodeAreaFX.ScrollPosition getScrollPosition() {
            return scrollPosition;
        }

        public CodeAreaFX.CharRenderingMode getCharRenderingMode() {
