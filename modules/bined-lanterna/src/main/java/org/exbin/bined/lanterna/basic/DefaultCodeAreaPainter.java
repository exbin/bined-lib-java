...
public class DefaultCodeAreaPainter implements CodeAreaPainter, BasicColorsCapableCodeAreaPainter {
    ...
    public DefaultCodeAreaPainter(CodeAreaCore codeArea) {
        ...
        scrollPanel = new DefaultCodeAreaScrollPane((CodeAreaLanternaControl) codeArea, metrics, structure, dimensions, scrolling);
...
//        metrics.recomputeMetrics(g.getFontMetrics(font), charset);
    ...
    protected void recomputeScrollState() {
        ...
        int characterWidth = 1; //metrics.getCharacterWidth();
    ...
    public void paintHeader(Graphics g) {
        ...
        int characterWidth = 1; // metrics.getCharacterWidth();
        int rowHeight = 1; // metrics.getRowHeight();
        ...
        if (viewMode == CodeAreaViewMode.DUAL || viewMode == CodeAreaViewMode.CODE_MATRIX) {
            ...
            for (int characterOnRow = skipToChar; characterOnRow < codeCharEnd; characterOnRow++) {
                ...
                if (!CodeAreaLanternaUtils.areSameColors(color, renderColor)) { // || !colorType.equals(renderColorType)
                    sequenceBreak = true;
                }
                if (sequenceBreak) {
                    ...
                    if (!CodeAreaLanternaUtils.areSameColors(color, renderColor)) {
                        renderColor = color;
                        g.setColor(color);
                    }
    ...
    public void paintRowPosition(Graphics g) {
        ...
        int rowHeight = 1; //metrics.getRowHeight();
        int characterWidth = 1; //metrics.getCharacterWidth();
        int subFontSpace = 0; //metrics.getSubFontSpace();
...
//
    ...
    public void paintRowText(Graphics g, long rowDataPosition, int rowPositionX, int rowPositionY) {
        ...
        for (int charOnRow = skipToChar; charOnRow < skipRestFromChar; charOnRow++) {
            ...
            if (sequenceBreak) {
                if (!CodeAreaLanternaUtils.areSameColors(lastColor, renderColor)) {
    ...
    protected void paintCursorRect(Graphics g, int cursorX, int cursorY, int width, int height, CursorRenderingMode renderingMode, DefaultCodeAreaCaret caret) {
    ...
    private void updateMirrorCursorRect(long dataPosition, CodeAreaSection section) {
        ...
        if (mirrorCursorPoint == null) {
            ...
            cursorDataCache.mirrorCursorRect.setBounds(mirrorCursorPoint.x, mirrorCursorPoint.y, /* metrics.getCharacterWidth() * */ (section == BasicCodeAreaSection.TEXT_PREVIEW ? codeType.getMaxDigitsForByte() : 1), 1 /*metrics.getRowHeight() */);
        }
    }
    ...
    protected void drawCenteredChars(Graphics g, char[] drawnChars, int charOffset, int length, int cellWidth, int positionX, int positionY) {
        ...
        while (pos < length) {
            ...
            int charWidth = 1; //metrics.getCharWidth(drawnChar);
...
//            }
    ...
    public void updateScrollBars() {
        int verticalScrollBarPolicy = CodeAreaLanternaUtils.getVerticalScrollBarPolicy(scrolling.getVerticalScrollBarVisibility());
        ...
        int horizontalScrollBarPolicy = CodeAreaLanternaUtils.getHorizontalScrollBarPolicy(scrolling.getHorizontalScrollBarVisibility());
    ...
    protected void horizontalExtentChanged() {
        scrollPanel.horizontalExtentChanged();
    }

    protected void verticalExtentChanged() {
